...
$PAGESFILE = dirname(__FILE__)."/zoom_pages.zdat";
$TITLESFILE = dirname(__FILE__)."/zoom_titles.zdat";
$SPELLINGFILE = dirname(__FILE__)."/zoom_spelling.zdat";
$DESCRIPTIONSFILE = dirname(__FILE__)."/zoom_descriptions.zdat";
$CATSFILE = dirname(__FILE__)."/zoom_cats.zdat";
$CATPAGESFILE = dirname(__FILE__)."/zoom_catpages.zdat";
$DATETIMEFILE = dirname(__FILE__)."/zoom_datetime.zdat";
...
if (!file_exists($SETTINGSFILE) || !file_exists($WORDMAPFILE) || !file_exists($DICTIONARYFILE)
    || !file_exists($PAGESFILE) || !file_exists($TITLESFILE))
{
...
function PrintEndOfTemplate($template)
{
    ...
    if ($ZoomInfo == 1)
        print("<p align="center"><small>" . $STR_POWEREDBY . " <a href=\"http://www.wrensoft.com/zoom/\" target=\"_blank\"><b>Zoom Search Engine</b></a></small></p>");
...
function PrintHighlightDescription($line)
{
    ...
    {
        if (strlen($SearchWords[$i]) < 1)
            continue;

        // replace with marker text, assumes [;:] and [:;] is not the search text...
        if ($SearchAsSubstring == 1)
            $res = preg_replace("/(" .$SearchWords[$i] . ")/i", "[;:]$1[:;]", $res);
        else
            $res = preg_replace("/(\W|\A|\b)(" .$SearchWords[$i] . ")(\W|\Z|\b)/i", "$1[;:]$2[:;]$3", $res);
    }
...
if ($UseCats)
{
    if (file_exists($CATSFILE) && file_exists($CATPAGESFILE))
    {
        $catnames = file($CATSFILE);
        $catpages = file($CATPAGESFILE);
    }
    else
    {
        print("Zoom config error: Missing file(s) zoom_cats.zdat and zoom_catpages.zdat required for category enabled search mode");
        return;
    }
}
...
{
    $fp_datetime = fopen ($DATETIMEFILE, "rt");
    $i = 0;
    while (!feof($fp_datetime) && $i < $NumPages)
    {
        $dateline = fgets($fp_datetime, $MaxKeyWordLineLen);
        if (strlen($dateline) > 0)
        {
            $datetime[$i] = strtotime($dateline);
            $i++;
        }
    }
    if ($i < $NumPages)
...
// strip slashes, sloshes, consecutive spaces, parenthesis, etc.
...
preg_match_all("/\"(.*?)\"|[^\\s\"]+/", $query, $SearchWords);
...
//Print heading
print("<div class=\"searchheading\">" . $STR_RESULTS_FOR . " " . htmlspecialchars($query));
if ($UseCats) {
    if ($cat == -1)
                ...
                if ($ExactPhrase == 1)
                {
                    ...
                    do
                    {
                        for ($xi = 0; $xi < $num_phrase_terms; $xi++)
                        {
                            ...
                            if (wordcasecmp($dict[$xword_id][0], $phrase_terms[$xi]) != 0)
                                break;
...
if ($SkippedWords > 0)
{
    print "<i>" . $STR_SKIPPED_FOLLOWING_WORDS . " " . $SkippedOutputStr . "<br />\n";
...
{
    if ($and == 1 && $numwords > 1)
        print "<br />" . $STR_POSSIBLY_GET_MORE_RESULTS . " <a href=\"".$SelfURL."?zoom_query=".$query_out."&amp;zoom_page=".$page."&amp;zoom_per_page=".$per_page."&amp;zoom_cat=".$cat."&amp;zoom_and=0&amp;zoom_sort=".$sort."\">". $STR_ANY_OF_TERMS . "</a>.<br />";
    else if ($UseCats && $cat != -1)
        print "<br />" . $STR_POSSIBLY_GET_MORE_RESULTS . " <a href=\"".$SelfURL."?zoom_query=".$query_out."&amp;zoom_page=".$page."&amp;zoom_per_page=".$per_page."&amp;zoom_cat=-1&amp;zoom_and=".$and."&amp;zoom_sort=".$sort."\">" . $STR_ALL_CATS . "</a>.<br />";
}
print "</div>\n";
...
if ($Spelling == 1)
{
    ...
    {
        print "<br />" . $STR_DIDYOUMEAN . " <a href=\"".$SelfURL."?zoom_query=".urlencode($SuggestStr)."&amp;zoom_page=".$page."&amp;zoom_per_page=".$per_page."&amp;zoom_cat=".$cat."&amp;zoom_and=0&amp;zoom_sort=".$sort."\">". $SuggestStr . "</a>";
        if (strlen($word2) > 0)
        	print " or <a href=\"".$SelfURL."?zoom_query=".urlencode($word2)."&amp;zoom_page=".$page."&amp;zoom_per_page=".$per_page."&amp;zoom_cat=".$cat."&amp;zoom_and=0&amp;zoom_sort=".$sort."\">". $word2 . "</a>";
        if (strlen($word3) > 0)
        	print " or <a href=\"".$SelfURL."?zoom_query=".urlencode($word3)."&amp;zoom_page=".$page."&amp;zoom_per_page=".$per_page."&amp;zoom_cat=".$cat."&amp;zoom_and=0&amp;zoom_sort=".$sort."\">". $word3 . "</a>";
        print "?<br />";
    }
...
if ($num_pages > 1)
    print "<br />" . $num_pages . " " . $STR_PAGES_OF_RESULTS . "<br />\n";
...
if ($matches > 1)
{
    if ($UseDateTime == 1)
    {
        ...
        if ($sort == 1)
            print("<a href=\"".$SelfURL."?zoom_query=".$query_out."&amp;zoom_page=".$page."&amp;zoom_per_page=".$per_page."&amp;zoom_cat=".$cat."&amp;zoom_and=".$and."&amp;zoom_sort=0\">". $STR_SORTBY_RELEVANCE . "</a> / <b>". $STR_SORTEDBY_DATE . "</b>");
        else
            print("<b>". $STR_SORTEDBY_RELEVANCE . "</b> / <a href=\"".$SelfURL."?zoom_query=".$query_out."&amp;zoom_page=".$page."&amp;zoom_per_page=".$per_page."&amp;zoom_cat=".$cat."&amp;zoom_and=".$and."&amp;zoom_sort=1\">". $STR_SORTBY_DATE . "</a>");
        print("</div>");
    }
}
...
while ($arrayline < $matches && $arrayline < $result_limit)
{
    ...
    {
		if ($SearchAsSubstring == 1)
			$urlLink = AddParamToURL($urlLink, "zoom_highlightsub=".$query_out);
		else
			$urlLink = AddParamToURL($urlLink, "zoom_highlight=".$query_out);
    }
    if ($DisplayTitle == 1)
    {
        ...
        print "<a href=\"".$urlLink."\"" . $target . ">".rtrim($urls[$ipage])."</a>";
...
if ($num_pages > 1) {
    ...
    print "<p></p>\n" . $STR_RESULT_PAGES . " ";
    if ($page > 1)
        print "<a href=\"".$SelfURL."?zoom_query=".$query_out."&amp;zoom_page=".($page-1)."&amp;zoom_per_page=".$per_page."&amp;zoom_cat=".$cat."&amp;zoom_and=".$and."&amp;zoom_sort=".$sort."\">&lt;&lt; " . $STR_RESULT_PAGES_PREVIOUS . "</a> ";

    for ($i = $start_range; $i <= $end_range; $i++) {
        if ($i == $page) {
            print $page." ";
        } else {
            print "<a href=\"".$SelfURL."?zoom_query=".$query_out."&amp;zoom_page=".($i)."&amp;zoom_per_page=".$per_page."&amp;zoom_cat=".$cat."&amp;zoom_and=".$and."&amp;zoom_sort=".$sort."\">".$i."</a> ";
        }
    }

    if ($page != $num_pages)
        print "<a href=\"".$SelfURL."?zoom_query=".$query_out."&amp;zoom_page=".($page+1)."&amp;zoom_per_page=".$per_page."&amp;zoom_cat=".$cat."&amp;zoom_and=".$and."&amp;zoom_sort=".$sort."\">" . $STR_RESULT_PAGES_NEXT . " &gt;&gt;</a> ";
}
...
if ($Timing == 1 || $Logging == 1) {
    ...
    if ($Timing == 1)
        print "<br /><br /><small>" . $STR_SEARCH_TOOK . " " . $timetaken . " " . $STR_SECONDS . "</small><br />\n";
}
...
if ($Logging == 1)
{
    ...
    $LogString = Date("Y-m-d, H:i:s") . ", " . $_SERVER['REMOTE_ADDR'] . ", \"" .$LogQuery  . "\", Matches = " . $matches;
    if ($and == 1)
    ...
    else
    {
        ...
        {
            $logCatStr = trim($catnames[$cat]);
            $logCatStr = str_replace("\"", "\"\"", $logCatStr);
            $LogString = $LogString . ", \"Cat = " . $logCatStr . "\"";
        }
    }
